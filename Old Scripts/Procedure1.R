...
gibbs <- function(niter, burn_in, F_t, nu, rho_params, returns,
  ...
  delta[1,] = rdirichlet(1, alpha = nu) #c(0.1,0.05,0.05,0.8) #rep(0.25,4)
  rho[1,] = runif(num_countries,0,1)
...
n_trials = 6; nreps = 2000; burn_in = 1000
nu = rep(0.5,4)
start_time <- Sys.time()
a_results = matrix(0, nrow = n_trials*(nreps-burn_in), ncol = num_countries)
b_results = matrix(0, nrow = n_trials*(nreps-burn_in), ncol = 3*num_countries)
delta_results = matrix(0, nrow = n_trials*(nreps-burn_in), ncol = 4)
rho_results = matrix(0, nrow = n_trials*(nreps-burn_in), ncol = num_countries)

for(i in 1:n_trials){
  print(i)
  list[alpha_results_temp, beta_results_temp, delta_results_temp, rho_results_temp] = 
    gibbs(niter = nreps, burn_in, F_t,
          nu, rho_params = c(2,2), returns,
          m_min_network[[1]], m_plus_network[[1]],
          v_min_network[[1]], v_plus_network[[1]])
  if(burn_in > 0){
    alpha_results_fil = tail(alpha_results_temp, -burn_in)
    beta_results_fil = tail(t(beta_results_temp), -burn_in)
    delta_results_fil = tail(delta_results_temp, -burn_in)
    rho_results_fil = tail(rho_results_temp, -burn_in)
  } else if(burn_in == 0){
    alpha_results_fil = alpha_results_temp
    beta_results_fil = t(beta_results_temp)
    delta_results_fil = delta_results_temp
    rho_results_fil = rho_results_temp
  }
  # print((i-1)*(nreps-burn_in)+1)
  # print(i*(nreps-burn_in))
  # print("Temp results:")
  # print(alpha_results_fil)
  # print(beta_results_fil)
  # print(delta_results_fil)
  # print(rho_results_fil)
  cat(nrow(alpha_results_fil), ncol(alpha_results_fil), "\n")
  cat((i-1)*(nreps-burn_in)+1, "\n")
  cat(i*(nreps-burn_in), "\n")
  a_results[((i-1)*(nreps-burn_in)+1):(i*(nreps-burn_in)),] = alpha_results_fil
  b_results[((i-1)*(nreps-burn_in)+1):(i*(nreps-burn_in)),] = beta_results_fil
  delta_results[((i-1)*(nreps-burn_in)+1):(i*(nreps-burn_in)),] = delta_results_fil
  rho_results[((i-1)*(nreps-burn_in)+1):(i*(nreps-burn_in)),] = rho_results_fil
...
plots_everything <- function(){
  ...
  for(i in 1:3){ # ALPHA PLOTS
    ...
    hist(Trim(a_results[,i], trim = 0.05), freq = F,
         breaks = seq(min(Trim(a_results[,i], trim = 0.05)-0.1),
                      max(Trim(a_results[,i], trim = 0.05)+0.1), by=0.05),
  ...
  for(i in 1:6){ # RHO PLOTS
    hist(rho_results[,i], freq = F, col = "blue",
         breaks = seq(min(rho_results[,i]) - 0.1,
                      max(rho_results[,i]) + 0.1, by=0.005),
         main = paste0("Histogram of results of rho", i))
    abline(v = rho_DGP[i], col="red")
  }
  for(i in 1:3){ # BETA PLOTS
    hist(Trim(b_results[,i], trim = 0.01), freq = F, col = "blue",
         breaks = seq(min(Trim(b_results[,i], trim = 0.01) - 3),
                      max(Trim(b_results[,i],trim = 0.01) + 3), by=0.1),
