...
source("uni.slice.R")
source("sample_rho.R")
source("log_target_density_delta.R")
source("log_proposal_delta.R")
source("metropolis_delta.R")
source("sim_data.R")
...
gibbs <- function(switch_1, switch_2, niter, F_t, nu, rho_params, returns,
  ...
  for(i in 2:niter){
    ...
    if(i == switch_2){
      ...
      print(colMeans(delta[(switch_2-2000):switch_2,]))
    ...
    # } else if(i >= switch_1){
    #    alpha_star[i,] = colMeans(alpha_star[(switch_1-10):(switch_1-1),])
    }
    ...
    # } else if(i >= switch_1){
    #     vec_beta_star[,i] = rowMeans(vec_beta_star[,(switch_1-10):(switch_1-1)])
    }
    ...
    #vec_beta_star[,i] = vec_beta_star[,1]
    #cat("vec_beta_star[,i] \n", vec_beta_star[,i], "\n")
    ...
    ##### DELTA #####
    if(i >= switch_1 && i != switch_2){
    #if(i >= switch_2){ 
      delta[i,] = metropolis_delta(start = delta[i-1,],
                                     alpha[i,], beta_tilde[,i], rho[i-1,], 
                                     Sigma_eta, nu, F_t, y,
                                     m_min_network, m_plus_network,
                                     v_min_network, v_plus_network)
      print(delta[i,])
        ...
        delta[i,] = rep(0.25,4)
          ...
          delta[i,] = colMeans(delta[(switch_2-1000):(switch_2-1),])
    }
    ...
    if(i >= switch_2 ){
    # if(i >= switch_1  && i < switch_2){
      rho[i,] = sample_rho(rho[i-1,], alpha[i,], beta_tilde[,i], 
  ...
  return(list(alpha_star, vec_beta_star, delta[switch_1:niter,], rho[switch_2:(niter-1),]))
...
for(i in 1:n_trials){
  list[alpha_results, beta_results, delta_results, rho_results] = 
    gibbs(switch_1 = 2000, switch_2 = 6000, niter = 6750,
