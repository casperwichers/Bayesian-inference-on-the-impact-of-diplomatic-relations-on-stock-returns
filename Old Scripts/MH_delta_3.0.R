...
# delta = c(0.6,0.1,0.15,0.15) #Arbitrary, can choose and see if algo converges to this value
# 
...
log_target_density_delta <- function(delta, alpha, beta_tilde, Sigma,
  ...
  summation_term = 0
  for(t in 1:T){
    A[[t]] = diag(num_countries) - rho * (delta[1]*m_min_network + delta[2]*m_plus_network
...
#############################################################################
#############################################################################
metropolis_delta <- function(start, nreps, burn_in,
...
# n_reps=10000; burn_in_period = 1000; n_trials = 1
# nu = rep(3,4)
...
#                     max(thetas_fil[[1]][,i])+0.05, by=0.005),
#        col = "lightblue",
