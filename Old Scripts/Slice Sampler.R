...
uni.slice <- function (x0, g, w=1, m=Inf, lower=-Inf, upper=+Inf, gx0=NULL,
                       alpha, beta_tilde, Sigma_eta, delta,
                       returns, F_t,
                       m_min_network, m_plus_network,
                       v_min_network, v_plus_network,
                       prior_dhs_rho)
  # Here, I added that all necessary variables are also parametrized
{
...
sample_rho <- function(rho, alpha, beta_tilde, Sigma_eta, 
                      delta, returns, F_t,
                      m_min_network, m_plus_network,
                      v_min_network, v_plus_network,
                      prior_dhs_rho){
  ...
  for(j in 1:p){
    ...
    if(!is.null(prior_dhs_rho)){
      # Check to make sure the prior params make sense
      if(length(prior_dhs_rho) != 2) stop('prior_dhs_phi must be a numeric vector of length 2')
      
      x = (rho[j] + 1)/2 # ~ Beta(prior_dhs_phi[1], prior_dhs_phi[2])
      
      ##########################################################################
      #print(x)
      # Slice sampler when using Beta prior:
      x = uni.slice(x0 = x,
                    g = function(x, alpha,beta_tilde,
                                 Sigma_eta, delta, 
                                 returns, F_t, 
                                 m_min_network, m_plus_network,
                                 v_min_network,v_plus_network,
                      ...
                      #rho = 2*x-1
                      A = list(); Sigma = list(); F_tilde = list()#;x = list() 
                      ...
                      summation_term = 0
                      for(t in 1:T){
                        A[[t]] = diag(6) - x * (delta[1]*m_min_network + delta[2]*m_plus_network
                        ...
                        summation_term = (summation_term + (t(temp) %*% solve(Sigma[[t]]) %*% (temp)))
                      }
                      
                      log_llik = log(det(Sigma[[t]])) - summation_term
                      log_prior = dbeta(x, shape1 = prior_dhs_rho[1], shape2 = prior_dhs_rho[2], log = TRUE)
...
for(i in 1:n_trials){ #can change rho arbitrarily, all samples will be below this value
  print(i)
  rhos[i] = sample_rho(rho = 0.99, alpha, beta_tilde, Sigma_eta,
                       delta, returns, F_t,
                       m_min_network[[1]], m_plus_network[[1]],
                       v_min_network[[1]], v_plus_network[[1]],
                       prior_dhs_rho = c(3,2)) #these can also be adjusted to ones preference
}
