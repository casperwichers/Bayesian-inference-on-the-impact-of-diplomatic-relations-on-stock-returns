...
uni.slice <- function (x0, g, w=1, m=Inf, lower=-Inf, upper=+Inf, gx0=NULL,
                       alpha, beta_tilde, j, Sigma, delta, rho,
                       returns, F_t,
  ...
  # Here, I added that all necessary variables are also parameterized
{
  ...
  if (is.infinite(m))  # no limit on number of steps
  {
    ...
    { if (L<=lower) break
      #uni.slice.evals <<- uni.slice.evals + 1
      if (g(L,alpha,beta_tilde, j, #previously: if (g(L)<=logy) break
            Sigma, delta, rho,
            returns, F_t, 
            m_min_network, m_plus_network,
            v_min_network,v_plus_network,
    ...
    { if (R>=upper) break
      #uni.slice.evals <<- uni.slice.evals + 1
      if (g(R,alpha,beta_tilde, j, #previously: #if (g(R)<=logy) break
            Sigma, delta, rho,
            returns, F_t, 
            m_min_network, m_plus_network,
            v_min_network,v_plus_network,
  ...
  repeat
  {
    ...
    #uni.slice.evals <<- uni.slice.evals + 1
    gx1 <- g(x1,alpha,beta_tilde, j, # previously: g(x1)
             Sigma, delta, rho,
             returns, F_t, 
             m_min_network, m_plus_network,
             v_min_network,v_plus_network,
...
sample_rho <- function(rho, alpha, beta_tilde, Sigma, 
  ...
  for(j in 1:p){
    ...
    if(!is.null(rho_params)){
      ...
      u = uni.slice(x0 = u,
          g = function(u, alpha, beta_tilde, j,
                       Sigma, delta, rho,
                       returns, F_t, 
                       m_min_network, m_plus_network,
                       v_min_network,v_plus_network,
            ...
            A = list(); F_tilde = list()#; Sigma = list()
            
            y_tilde = matrix(nrow = T, ncol = num_countries) #y_tilde = y_t - alpha
            for(i in 1:ncol(returns)){
              y_tilde[,i] = returns[,i]-alpha[i]
            }
            log_llik = 0;summation_term = 0;
            for(t in 1:T){
              A[[t]] = diag(num_countries) - 
                diag(rho_star) %*% (delta[1]*m_min_network + delta[2]*m_plus_network
            ...
            log_prior = dbeta(u, shape1 = rho_params[1],
                        shape2 = rho_params[2], log = TRUE)
            log_dens = log_llik + log_prior
            #cat("log llik: \n", log_llik, "\n")
            #cat("log llik: \n", log_llik, "\n")
            return(log_dens[1,1])
          },
          w=1, m=Inf, lower = 0, upper = 1, gx0 = NULL,
          alpha, beta_tilde, j, Sigma, delta, rho,
          returns, F_t,
          m_min_network, m_plus_network,
          v_min_network, v_plus_network,
