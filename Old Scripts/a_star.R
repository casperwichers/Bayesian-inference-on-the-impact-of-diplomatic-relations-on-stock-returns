...
##################  SIMULATE DATA ##################
alpha_DGP = mvrnorm(n = 1, mu = rep(0,num_countries), Sigma = diag(num_countries))
...
alpha_star_DGP = solve(A_DGP) %*% alpha_DGP
...
n_trials = 1; nreps = 2000; burn_in = 500
nu = rep(0.5,4)
start_time <- Sys.time()
for(i in 1:n_trials){
