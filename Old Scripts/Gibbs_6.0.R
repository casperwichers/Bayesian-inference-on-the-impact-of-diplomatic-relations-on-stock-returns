...
gibbs <- function(niter, burn_in, F_t, nu, rho_params, returns,
  ...
  for(i in 2:niter){
    ...
    delta[i,] = metropolis_delta(start = delta[i-1,],
                                 alpha[i,], beta_tilde[,i], rho[i-1,], Sigma_eta,
                                 nu, F_t, y,
                                 m_min_network, m_plus_network,
                                 v_min_network, v_plus_network)
    #delta[i,] = delta_DGP
    ...
    # rho[i,] = sample_rho(rho[i-1,], alpha[i,], beta_tilde[,i], Sigma_eta,
    #                      delta[i,], y, F_t,
    #                      m_min_network, m_plus_network,
    #                      v_min_network, v_plus_network,
    #                      rho_params)
  ...
  return(list(alpha_star, vec_beta_star, delta, rho, acceptance_rate))
}
##################  CHOOSE ALGORITHM SETUP ##################
n_trials = 1; nreps = 7500; burn_in = 2500
results_mat = matrix(data = 0, nrow = n_trials, ncol = 5)
nu = rep(3,4)
start_time <- Sys.time()
for(i in 1:n_trials){
  ...
  results_mat[i,2:5] = colMeans(delta_results_fil)
  nu = nu + 1
