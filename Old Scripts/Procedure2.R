...
##################  RUN THE ALGORITHM ##################
gibbs <- function(niter, switch, burn_in, F_t, nu, rho_params, returns,
  ...
  for(i in 2:niter){
    ...
    if(i < switch){
      ...
      vec_beta_star[,i] = vec_beta_star[,(switch-1)]
    }
...
n_trials = 1; nreps = 3000; switch_val = 2000; burned_samples = 1000
nu = rep(0.5,4)
start_time <- Sys.time()
for(i in 1:n_trials){
  list[alpha_results, beta_results, delta_results, rho_results] = 
    gibbs(niter = nreps, switch = switch_val, burn_in = burned_samples, 
          F_t, nu, rho_params = c(2,2), returns,
          m_min_network[[1]], m_plus_network[[1]],
          v_min_network[[1]], v_plus_network[[1]])
  if(burned_samples > 0){
    alpha_results_fil = tail(alpha_results, -burned_samples)
    beta_results_fil = tail(t(beta_results), -burned_samples)
    delta_results_fil = tail(delta_results, -burned_samples)
    rho_results_fil = tail(rho_results, -burned_samples)
  } else if(burned_samples == 0){
