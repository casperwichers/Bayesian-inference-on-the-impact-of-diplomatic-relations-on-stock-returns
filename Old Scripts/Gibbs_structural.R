...
gibbs <- function(niter, burn_in, F_t, nu, rho_params, returns,
  ...
  ##################  DRAW INITIAL VALUES ##################
  alpha_star[1,] = mvrnorm(1, mu = mu_alpha, Sigma = Sigma_alpha)
  ...
  for(i in 2:niter){
    ...
    ##### ALPHA #####
    Sigma = solve(A) %*% Sigma_eta[[i-1]] %*% t(solve(A))
    A0_alpha = solve(Sigma_alpha)
    A1_alpha = T*t(solve(A)) %*% solve(Sigma) %*% solve(A)
    An_alpha = A0_alpha + A1_alpha
    b0_alpha = solve(Sigma_alpha) %*% mu_alpha
    b1_alpha = T*t(solve(A)) %*% solve(Sigma) %*% colMeans(y_tilde)
    ...
    for (t in 1:T){
      gamma_t = solve(A) %*% x[[t]]
      A1_beta_tilde = A1_beta_tilde + t(gamma_t) %*% solve(Sigma) %*% gamma_t 
      b1_beta_tilde = b1_beta_tilde + t(gamma_t) %*% solve(Sigma) %*% y_star[t,]
      #A1_beta_tilde = A1_beta_tilde + t(x[[t]]) %*% solve(Sigma[[i-1]]) %*% x[[t]]
      #b1_beta_tilde = b1_beta_tilde + t(x[[t]]) %*% solve(Sigma[[i-1]]) %*% y_star[t,]
    }
    ...
    # #print(Sigma_eta[[i]])
    Sigma_eta[[i]] = diag(num_countries)
