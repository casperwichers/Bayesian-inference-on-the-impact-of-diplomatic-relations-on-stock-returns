...
log_target_density_delta <- function(delta, alpha, beta_tilde, Sigma_eta,
  ...
  A = list(); Sigma = list();x = list(); F_tilde = list();
  ...
  log_dens = log(det(Sigma[[1]])) - summation_term/2 + log_prior_term
...
metropolis_delta <- function(start, nreps, burn_in = 500,
  ...
  for (i in 2:nreps){
    ...
    b = (log_target_density_delta(delta = theta_star, alpha, beta_tilde, Sigma_eta = Sigma_eta,
                              rho, returns, F_t, nu,
                              m_min_network, m_plus_network,
                              v_min_network, v_plus_network) 
         + log_proposal_delta(theta_star,nu))
    a = (log_target_density_delta(delta = theta[i-1,], alpha, beta_tilde, Sigma_eta = Sigma_eta,
                              rho, returns, F_t, nu,
                              m_min_network, m_plus_network,
                              v_min_network, v_plus_network) 
...
n_reps=100; burn_in_period = 0; n_trials = 1
nu = c(1,1,1,1)
