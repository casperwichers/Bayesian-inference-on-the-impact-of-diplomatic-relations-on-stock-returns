...
delta_ini = c(0.7,0.2,0.05,0.05) #Arbitrary, can choose and see if algo converges to this value
rho_ini =  0.8 #rep(0.6,num_countries) #idem
...
gibbs <- function(niter, burn_in, F_t, nu, rho_params, returns,
  ...
  for(i in 2:niter){
    ...
    # delta[i,] = metropolis_delta(start = delta[i-1,], nreps = 2, burn_in = 0,
...
for(i in 1:n_trials){
  ...
  for(i in 1:3){ # DELTA PLOTS
    ...
    #acf(delta_results_fil[,i], lag.max = 20, plot = TRUE)
  }
  hist(rho_results_fil, freq = F, breaks = seq(min(rho_results) - 0.5, max(rho_results) + 0.5, by=0.1))
  abline(v = rho_ini, col="red")
  for(i in 1:2){
  hist(beta_tilde_results_fil[,i], freq = F,
       breaks = seq(min(Trim(beta_tilde_results_fil[,i], trim = 0.01) - 3),
                    max(Trim(beta_tilde_results_fil[,i],trim = 0.01) + 3), by=0.1),
       main = paste0("Hist of sampled values of beta_tilde_1", i))
  abline(v = beta_tilde_ini[i], col = "red")
  }
  ##############################################################################
  # par(mfrow=c(6,2))
  # for(i in 1:2){ # ALPHA PLOTS
  #   plot(alpha_results_fil[,i], type = "l", lwd = 3,
  #        main = paste("Plot of MH values for",gsub(" ", "", paste("α", i))),
  #        ylab = "Value",
  #        col = "blue",
  ...
  #   abline(v = delta_ini[i], col="red")
  # }
  # plot(rho_results_fil, type = "l", lwd = 1, col = "blue", ylab = "Value",
  #      main = "Plot for sampled values of rho")
  # abline(h = rho_ini, col = "red")
  # hist(rho_results_fil, freq = F, breaks = seq(min(rho_results) - 0.5, max(rho_results) + 0.5, by=0.1))
  # abline(v = rho_ini, col="red")
  # plot(beta_tilde_results_fil[,1], type = "l", lwd = 1, col = "blue", ylab = "Value",
  #      main = "Hist of sampled values of beta_tilde_1")
  # abline(h = beta_tilde_ini[1], col = "red")
  # hist(beta_tilde_results_fil[,1], freq = F,
  #      breaks = seq(min(Trim(beta_tilde_results_fil[,1], trim = 0.01) - 3),
  #                   max(Trim(beta_tilde_results_fil[,1],trim = 0.01) + 3), by=0.1),
  #      main = "Hist of sampled values of beta_tilde_1")
  # abline(v = beta_tilde_ini[1], col = "red")
  #nu = nu + 0.5
  ##############################################################################
}
...
# if(burn_in > 0){
#   alpha_results_fil = tail(alpha_results, -burn_in)
#   beta_tilde_results_fil = tail(beta_tilde_results, -burn_in)
#   delta_results_fil = tail(delta_results, -burn_in)
#   rho_results_fil = tail(rho_results, -burn_in)
# } else if(burn_in == 0){
#   alpha_results_fil = alpha_results
#   beta_tilde_results_fil = beta_tilde_results
#   delta_results_fil = delta_results
#   rho_results_fil = rho_results
# }
...
#   trim = 0.05
#   hist(Trim(alpha_results_fil[,i], trim = 0.05), freq = F,
#        breaks = seq(min(Trim(alpha_results_fil[,i], trim = trim) - 0.5), 
#            max(Trim(alpha_results_fil[,i], trim = trim) + 0.5), by=0.1),
#        main = paste("Hist of MH values for",gsub(" ", "", paste("α", i)),
#                     "trimmed by",trim),
#        xlab = "Value",
#        col = "blue")
#   abline(v = alpha_ini[i], col="red")
...
# }
# 
# plot(rho_results, type = "l", lwd = 1, col = "blue", ylab = "Value",
#      main = "Plot for sampled values of rho")
# abline(h = rho_ini, col = "red")
# hist(rho_results_fil, freq = F, breaks = seq(min(rho_results_fil) - 0.5, max(rho_results_fil) + 0.5, by=0.1))
# abline(v = rho_ini, col="red")
# plot(beta_tilde_results[1,], type = "l", lwd = 1, col = "blue", ylab = "Value",
#      main = "Hist of sampled values of beta_tilde_1")
# abline(h = beta_tilde_ini[1], col = "red")
# hist(beta_tilde_results_fil[1,], freq = F,
#      main = "Hist of sampled values of beta_tilde_1")
...
#             ", start value nu = (0.25,0.25,0.25,0.25)",
#             ", alpha =",gsub(" ", "", paste("(",nu[1], ",",nu[2],",", nu[3],",",nu[4],")"))),
#       side = 3, line = -32, outer = TRUE)
