...
source("metropolis_doublestep.R")
source("sim_data.R")
...
gibbs <- function(switch_1, switch_2, niter, F_t, nu, rho_params, returns,
  ...
  for(i in 2:niter){
    ...
    if(i < switch_2){
      ...
      alpha_star[i,] = colMeans(alpha_star[(switch_2-10):(switch_2-1),])
    }
    ...
    if(i < switch_2){
      ...
      vec_beta_star[,i] = rowMeans(vec_beta_star[,(switch_2-10):(switch_2-1)])
    }
    ...
    if(i >= switch_1 && i < switch_2){
      ...
      delta[i,] = rep(0.25,4)
      # } else if(i >= switch_2){
      #      delta[i,] = delta[(switch_2-1),]
    }
    
    ##### RHO ##### 
    # Network A will be constructed within 'sample_rho()', so no need to update A already here
    if(i >= switch_2){
      list[delta[i,], rho[i,]] = metropolis_doublestep(delta[i-1,], rho[i-1,],
                                                     alpha[i,], beta_tilde[,i],
                                                     Sigma_eta, y, F_t,
                                                     m_min_network, m_plus_network,
                                                     v_min_network, v_plus_network,
                                                     nu, rho_params)
...
for(i in 1:n_trials){
  list[alpha_results, beta_results, delta_results, rho_results] = 
    gibbs(switch_1 = 3000, switch_2 = 7000, niter = 7500,
